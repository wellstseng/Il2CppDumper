...
| 2026-05-20 | `s5c-full` (主執行緒 strip-stub-only，0 subagent，0 baseline) | `dls.game/IGG.Game.Notifys` 全 namespace 155/155 (effective 154; placeholder 1) | 155 disk / 154 effective (本批新增) | ~15K (主執行緒) | ~5K (protobuf signature check + missing scan) | no | 掃 155 Annotated（0 baseline），總行數 4,988（S5 系列極小，僅 S5-H 13%）；分布 tiny 147 / small 7 / med 1（無 large/xlarge/huge）；先檢查 protobuf 簽名（grep `Google.Protobuf` / `pb::Message`）0 命中，確認非 protobuf 而是 event name `const string` 表（如 `AbTestCS2CLSyncNotify = "AbTestNotify_AbTestCS2CLSyncNotify"`）；strip-stub 完美保留所有 const 定義 | （1）原本擔心是 protobuf 需跳過（依使用者指示 dls.message protobuf 不處理），protobuf 簽名 grep 後確認非 protobuf；未來其他 dll 若 namespace 名含 Notify/Message 先 grep 確認；（2）event name const 表類 namespace effective 比例 99.4% 為 S5 系列最高；（3）S5-D/E/F/G/H/C 累計完成 dls.game 700 檔 (444 effective)，平均 74 effective/batch；S5 剩 S5-A 406 / S5-B 341 / S5-I 6,448 約 7,195 檔；（4）token cost rank: S5-C ~15K &lt; S5-H ~25K &lt; S5-G ~30K &lt; S5-E ~80K &lt; S5-F ~90K &lt; S5-D ~850K |
| 2026-05-20 | `s5b-full` (主執行緒 strip-stub-only，0 subagent，0 baseline) | `dls.game/IGG.Game.Data.Cache.Mail.Type` 全 namespace 341/341 (effective 300; placeholder 41) | 341 disk / 300 effective (本批新增) | ~30K (主執行緒) | ~8K (missing scan + protobuf check) | no | 掃 341 Annotated（0 baseline，0 protobuf），總行數 73,079；分布 tiny 3 / small 297 (87%) / med 40 / xlarge 1（無 large/huge）；strip-stub 一次跑 341；effective 比例 88% | （1）Mail.Type 為 mail envelope 資料結構 namespace，small 比例 87% 為 S5 系列最高（之前 S5-H Cache.IM tiny+small 89% 主要在 tiny；本批集中在 small 100-300 行）；（2）S5-D/E/F/G/H/C/B 累計完成 dls.game 1,041 檔 (744 effective)，平均 106 effective/batch；S5 剩 S5-A 406 / S5-I 6,448 約 6,854 檔；（3）本 session 連續完成 S5-E/F/G/H/C/B 共 923 檔 (659 effective)，全程主執行緒 0 LLM subagent；（4）token cost rank: S5-C ~15K &lt; S5-H ~25K &lt; S5-G ~30K ≈ S5-B ~30K &lt; S5-E ~80K &lt; S5-F ~90K &lt; S5-D ~850K |
| 2026-05-20 | `s5a-full` (主執行緒 strip-stub-only，0 subagent，0 baseline) | `dls.game/IGG.Game.Module.Activity.View` 全 namespace 406/406 (effective 52; placeholder 354) | 406 disk / 52 effective (本批新增) | ~40K (主執行緒) | ~12K (missing scan + protobuf check) | no | 掃 406 Annotated（0 baseline，16 個 Google.Protobuf 結構），總行數 769,751（S5 系列最大批）；分布 tiny 24 / small 28 / med 59 / large 112 / xlarge 116 / huge 67；UI Activity View 業務邏輯密度最高（73% 為 med+ 大檔）；strip-stub 一次跑 406；effective 比例 13% 為 S5 系列最低 | （1）S5 系列最大規模批次（769K 行 vs S5-F 135K）走純 strip-stub-only 仍能完整 disk-cover + marker clean，驗證配方 scalability；（2）effective 比例 13% 為 S5 系列最低，對應 UI View 業務邏輯密度最高（73% 為 med+ 大檔）；（3）S5-D/E/F/G/H/C/B/A 累計完成 dls.game 1,447 檔 (796 effective)，平均 ~100 effective/batch；S5 剩 S5-I 6,448 檔（需多 session 拆批）；（4）本 session 累計連續 7 個 batch (S5-E/F/G/H/C/B/A) 共 1,329 檔 (711 effective)，全程主執行緒 0 LLM subagent；（5）token cost rank: S5-C ~15K &lt; S5-H ~25K &lt; S5-G ~30K ≈ S5-B ~30K &lt; S5-A ~40K &lt; S5-E ~80K &lt; S5-F ~90K &lt; S5-D ~850K |
